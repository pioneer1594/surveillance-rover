...
# ================= Database Setup =================
# UPDATE these credentials to match your PostgreSQL installation
DATABASE_URL = "postgresql://postgres:shein123@localhost/SSR"
...
# ================= Records Directory Setup =================
RECORDS_DIR = Path(__file__).parent / "records"
RECORDS_DIR.mkdir(exist_ok=True)

# Serve recorded videos statically at /records/<filename>
app.mount("/records", StaticFiles(directory=str(RECORDS_DIR)), name="records")
...
class SystemState:
    def __init__(self):
        self.mode = "webcam"  # "webcam", "esp32", "demo"
        self.esp32_ip = "192.168.4.1"
            ...
            try:
                if current_mode == "demo":
                    ...
                    if esp32_response is None:
                        ...
                        try:
                            esp32_response = requests.get(stream_url, stream=True, timeout=5.0)
...
def move(payload: MovePayload):
    ...
    if state.mode == "esp32" and state.esp32_ip:
    #esp32_url = f"http://{state.esp32_ip}/action?go={direction}" change code for rover control
        command_map = {
            "forward": "F",
            "backward": "B",
            "left": "L",
...
def register(payload: RegisterPayload):
    db = SessionLocal()
    try:
        # နာမည်တူ ရှိ/မရှိ စစ်ဆေးခြင်း
        existing_user = db.query(User).filter(User.username == payload.username).first()
        if existing_user:
            raise HTTPException(status_code=400, detail="Username already exists")
            
        # User အသစ်ကို Password Hash ပြီး သိမ်းခြင်း (password_hash ဟု ပြင်ထားသည်)
        new_user = User(
            username=payload.username,
            password_hash=get_password_hash(payload.password)
        )
...
def login(payload: LoginPayload):
    db = SessionLocal()
    try:
        # ၁။ User ရှိ/မရှိ စစ်ဆေးခြင်း
        user = db.query(User).filter(User.username == payload.username).first()
        if not user:
            raise HTTPException(status_code=401, detail="ACCESS DENIED: User not found.")
            
        # ၂။ ကိုယ်ပိုင် Password မှန်/မမှန် စစ်ဆေးခြင်း (password_hash ဟု ပြင်ထားသည်)
        if not verify_password(payload.password, user.password_hash):
            raise HTTPException(status_code=401, detail="ACCESS DENIED: Incorrect password.")
            
        # ၃။ ကား Wi-Fi Password မှန်/မမှန် စစ်ဆေးခြင်း
        car = db.query(Car).filter(Car.wifi_password == payload.car_wifi).first()
        if not car:
            raise HTTPException(status_code=401, detail="ACCESS DENIED: Incorrect Car Wi-Fi Password.")
...
@app.get("/api/videos")
def list_videos():
    """Return a sorted list of available .webm recordings from the records/ folder."""
    try:
        files = sorted(
            [f for f in os.listdir(RECORDS_DIR) if f.lower().endswith(".webm")],
